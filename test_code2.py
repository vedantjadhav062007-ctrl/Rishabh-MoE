...
print("Loading Golden Checkpoint to CPU RAM...", flush=True)
ckpt = torch.load("/opt/rishabh/ckpt_interrupted_step_8030.pt", map_location='cpu')
...
print("Moving Brain to GPU...", flush=True)
model.cuda()
print("Brain loaded successfully!", flush=True)

def encode_text(text):
    bytes_data = [ord(c) % 32000 for c in text]
    return torch.tensor([bytes_data], dtype=torch.long).cuda()
