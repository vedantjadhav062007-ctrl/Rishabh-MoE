...
print("[System] Loading rl_ckpt_4200.pt to CPU...", flush=True)
config = RishabhConfig()
model = Rishabh(config).to(torch.bfloat16)
checkpoint = torch.load('/opt/rishabh_experimental/rl_ckpt_4200.pt', map_location='cpu', weights_only=True)
...
# Generate 4 distinct thoughts to mimic RL
